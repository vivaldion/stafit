...
7. запускать файл с помощью команды, где economic1.csv economic2.csv имена файлов, которые необходимо прочесть, report.txt имя файла, куда будет сохраняться отчет. последний аргумент не является обязательным, его можно пропустить(average-gdp.txt по умолчанию)
